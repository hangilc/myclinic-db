...
exports.setChargeValue = function(conn, visitId, chargeValue, done){
	exports.findCharge(conn, visitId, function(err, charge){
		if( err ){
			done(err);
			return;
		}
		if( charge === null ){
			exports.insertCharge(conn, {visit_id: visitId, charge: chargeValue}, done);
		} else {
			exports.updateCharge(conn, {visit_id: visitId, charge: chargeValue}, done);
		}
	});
};