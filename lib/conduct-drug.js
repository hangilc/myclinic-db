...
exports.listConductDrugsForConduct = function(conn, conductId, cb){
	var sql = "select * from visit_conduct_drug where visit_conduct_id = ? order by id";
	var args = [conductId];
	conn.query(sql,args, cb);
};

exports.countConductDrugsForConduct = function(conn, conductId, cb){
	var sql = "select count(*) as c from visit_conduct_drug where visit_conduct_id = ?";
	var args = [conductId];
	conn.query(sql, args, function(err, result){
		if( err ){
			cb(err);
