...
exports.listDrugsForVisit = function(conn, visitId, cb){
	var sql = "select * from visit_drug where visit_id = ? order by drug_id";
	var args = [visitId];
	conn.query(sql, args, function(err, result){
		if( err ){
			cb(err);
...
exports.countUnprescribedDrugsForVisit = function(conn, visitId, cb){
	var sql = "select count(*) as c from visit_drug where visit_id = ? and d_prescribed = 0";
	var args = [visitId];
	conn.query(sql, args, function(err, result){
		if( err ){
			cb(err);
...
exports.listIyakuhincodesByPatient = function(conn, patientId, cb){
    var sql = "select d.d_iyakuhincode from visit_drug d, visit v " +
        " where d.visit_id = v.visit_id and v.patient_id = ? " +
        " group by d.d_iyakuhincode";
    var args = [patientId];
    conn.query(sql, args, function(err, result){
    	if( err ){
    		cb(err);
    	...
    	var list = result.map(function(d){
    		return d.d_iyakuhincode;
    	});
    	cb(undefined, list);
    });
};

exports.countVisitsByIyakuhincode = function(conn, patientId, iyakuhincode, cb){
    var sql = "select count(*) as c from (select d.visit_id from visit_drug d, visit v " + 
        " where d.visit_id = v.visit_id and v.patient_id = ? " +
        " and d.d_iyakuhincode = ? group by d.visit_id) as t ";
    var args = [patientId, iyakuhincode];
    conn.query(sql, args, function(err, result){
    	if( err ){
    		cb(err);
    		return;
    	...
    	if( result.length !== 1 ){
    		cb("cannot happen in countVisitsByIyakuhincode: " + result.length);
    		return;
    	}
    	cb(undefined, result[0].c);
    })
...
exports.listVisitIdsByIyakuhincode = function(conn, patientId, iyakuhincode, offset, count, cb){
    var sql = "select distinct d.visit_id as id from visit_drug d, visit v " +
        " where d.visit_id = v.visit_id and v.patient_id = ? " +
        " and d.d_iyakuhincode = ? order by d.visit_id desc " +
        " limit ?,? ";
    var args = [patientId, iyakuhincode, offset, count];
    conn.query(sql, args, function(err, result){
    	if( err ){
    		done(err);
    		return;
