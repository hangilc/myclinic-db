...
function getDiseaseWithMaster(conn, diseaseId, cb){
    var sql = "select d.*, m.* from disease d, shoubyoumei_master_arch m " + 
        " where d.disease_id = ? " + 
        " and d.shoubyoumeicode = m.shoubyoumeicode " +
        " and m.valid_from <= d.start_date " +
    ...
    conn.query(sql, args, function(err, result){
        ...
        if( result.length !== 1 ){
            cb("getDiseaseWithMaster failed: " + result.length);
            return;
        }
...
exports.getFullDisease = function(conn, diseaseId, cb){
    var disease;
    conti.exec([
        function(done){
            getDiseaseWithMaster(conn, diseaseId, function(err, result){
                if( err ){
                    done(err);
                    return;
                }
