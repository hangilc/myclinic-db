...
exports.searchIyakuhinMaster = function(conn, text, at, cb){
    var sql = "select * from iyakuhin_master_arch " +
    ...
    conn.query(sql, args, function(err, result){
    	...
    	result.forEach(util.deleteUnusedIyakuhinMasterColumn);
    	cb(undefined, result);
    })
};

exports.getNamesOfIyakuhin = function(conn, iyakuhincode, cb){
    var sql = "select name, yomi from iyakuhin_master_arch " + 
        " where iyakuhincode = ? limit 1";
